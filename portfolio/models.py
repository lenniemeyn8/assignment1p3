...
class Stock(models.Model):
    ...
    def initial_stock_value(self):
        return round(decimal.Decimal(self.shares * self.purchase_price), 2)

    def current_stock_price(self):
        symbol_f = str(self.symbol)
        main_api = 'https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol='
        api_key = '&interval=1min&apikey=Y74POR3HY4JRFIN9'
    ...
    def current_stock_value(self):
        ...
        open_price = float(json_data["Time Series (1min)"][mkt_dt]["1. open"])
        share_value = open_price
        return round(decimal.Decimal(float(share_value) * float(self.shares)), 2)
